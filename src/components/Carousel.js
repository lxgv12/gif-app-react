import React from 'react'

const Carousel = () => {
    return (
        <>
            <div className="container" style={{ marginTop: "5px" }}>
                <div id="FakeShopCarouselInterval" className="carousel slide" data-bs-ride="carousel">
                    <div className="carousel-inner">
                        <div className="carousel-item active" data-bs-interval="2000">
                            <img src="banners/1.jpg" className="d-block w-100" alt="1"  width={90} height={270}/>
                        </div>
                        <div className="carousel-item" data-bs-interval="2000">
                            <img src="banners/2.jpg" className="d-block w-100" alt="2" width={90} height={270}/>
                        </div>
                        <div className="carousel-item" data-bs-interval="2000">
                            <img src="banners/3.jpg" className="d-block w-100" alt="3" width={90} height={270}/>
                        </div>
                        <div className="carousel-item" data-bs-interval="2000">
                            <img src="banners/4.jpg" className="d-block w-100" alt="4" width={90} height={270}/>
                        </div>
                    </div>
                    <button className="carousel-control-prev" type="button" data-bs-target="#FakeShopCarouselInterval" data-bs-slide="prev">
                        <span className="carousel-control-prev-icon" aria-hidden="true"></span>
                        <span className="visually-hidden">Previous</span>
                    </button>
                    <button className="carousel-control-next" type="button" data-bs-target="#FakeShopCarouselInterval" data-bs-slide="next">
                        <span className="carousel-control-next-icon" aria-hidden="true"></span>
                        <span className="visually-hidden">Next</span>
                    </button>
                </div>
            </div>
        </>
    )
}

export default Carousel;